...
const String logoNetwork = '$siteUrl/wp-content/uploads/2025/07/1000073463.png';
...
class CategoryItem {
  ...
  final String slug;
  final IconData icon;
  const CategoryItem({required this.id, required this.name, required this.slug, required this.icon});
}
...
String postTitle(dynamic post) {
  try { return cleanHtml(post['title']['rendered'] ?? 'بدون عنوان'); } catch (_) { return 'بدون عنوان'; }
}

String postLink(dynamic post) {
  try { return post['link'] ?? ''; } catch (_) { return ''; }
}

String postImage(dynamic post) {
  try {
    final media = post['_embedded']?['wp:featuredmedia'];
    if (media is List && media.isNotEmpty) return media[0]['source_url'] ?? '';
  } catch (_) {}
  return '';
}
...
List<int> _gregorianToJalali(int gy, int gm, int gd) {
  ...
  var jm = 0;
  var jd = days + 1;
  for (var i = 0; i < 12; i++) { if (jd <= jdm[i]) { jm = i + 1; break; } jd -= jdm[i]; }
  return [jy, jm, jd];
}
...
class WordPressApi {
  static Future<List<dynamic>> getLatestPosts({int perPage = 6}) async {
    final response = await http.get(Uri.parse('$apiUrl/posts?per_page=$perPage&_embed'));
  ...
  static Future<List<dynamic>> getPostsByCategory(int categoryId, {int perPage = 20}) async {
    final response = await http.get(Uri.parse('$apiUrl/posts?categories=$categoryId&per_page=$perPage&_embed'));
    if (response.statusCode == 200) return json.decode(response.body);
...
class ItarbiatbadaniApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final base = ThemeData(
      ...
      colorScheme: ColorScheme.fromSeed(seedColor: goldColor, brightness: Brightness.dark),
      appBarTheme: const AppBarTheme(backgroundColor: backgroundColor, foregroundColor: textColor, elevation: 0),
    );
    ...
    return MaterialApp(
      ...
      theme: base.copyWith(textTheme: GoogleFonts.vazirmatnTextTheme(base.textTheme)),
      home: const MainPage(),
    );
  }
}
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> refresh() async {
    ...
    setState(() => postsFuture = posts);
    try { await posts; } catch (_) {}
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
            'دسترسی سریع به جدیدترین اخبار، منابع علمی، طرح درس، پاورپوینت‌های آموزشی و مطالب تخصصی تربیت بدنی و علوم ورزشی.',
            textAlign: TextAlign.right,
...
class ServicesSection extends StatelessWidget {
  ...
  static const List<ServiceItem> services = [
    ...
    ServiceItem(title: 'طرح درس', icon: Icons.assignment_outlined, url: '$siteUrl/product-category/طرح-درس/'),
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () => Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => ArticleWebViewPage(url: postLink(post), title: title),
        ),
      ),
      child: Container(
        ...
        slivers: [
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Column(
                children: staticCategories.map((c) => CategoryTile(category: c)).toList(),
              ),
            ),
          ),
        ],
...
class _CategoryPostsPageState extends State<CategoryPostsPage> {
  ...
  void initState() {
    super.initState();
    postsFuture = WordPressApi.getPostsByCategory(widget.category.id);
  }

  Future<void> _refresh() async {
    final future = WordPressApi.getPostsByCategory(widget.category.id);
    setState(() => postsFuture = future);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.category.name), backgroundColor: backgroundColor),
      body: RefreshIndicator(
        ...
        child: FutureBuilder<List<dynamic>>(
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.hasError) {
              return ErrorBox(
                message: 'دریافت مطالب این دسته با مشکل مواجه شد.\n${snapshot.error}',
                onRetry: _refresh,
              );
            }
            final posts = snapshot.data ?? [];
            if (posts.isEmpty) {
              return const _EmptyBox(text: 'مطلبی در این دسته پیدا نشد.');
            }
...
class NewsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('اخبار و رویدادها'), backgroundColor: backgroundColor),
      body: const Center(child: Text('صفحه اخبار به زودی...', style: TextStyle(color: textColor, fontSize: 16))),
    );
  }
}
...
class AccountPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('حساب من'), backgroundColor: backgroundColor),
      body: const Center(child: Text('صفحه حساب کاربری به زودی...', style: TextStyle(color: textColor, fontSize: 16))),
    );
  }
}
...
class SearchPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('جستجو'), backgroundColor: backgroundColor),
      body: const Center(child: Text('صفحه جستجو به زودی...', style: TextStyle(color: textColor, fontSize: 16))),
    );
  }
}
